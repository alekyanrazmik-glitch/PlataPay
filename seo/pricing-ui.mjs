...
function tierToData(tier) {
  ...
  return {
    label: text || 'Индивидуальный тариф',
    usd,
...
const pricingServices = SERVICES.map((service) => {
  const tiers = (service.tiers && service.tiers.length ? service.tiers : ['Индивидуальный тариф']).map(tierToData);
...
(function(){
  ...
  function updateCardPrice(card,s){
    ...
    if(!priceBox || !s) return;
    if(s.minRub){
      priceBox.innerHTML = '<span>от</span> <b>'+money(s.minRub)+'</b>';
    }
  }
  ...
  function tierCardsHtml(s){
    ...
    tiers.forEach(function(tier,idx){
      ...
      html += '<div class="pp-tariff-card">'
        + '<div class="pp-tariff-name">'+escapeHtml(tier.label)+'</div>'
        + '<div class="pp-tariff-usd">'+escapeHtml(usd)+'</div>'
        + '<div class="pp-tariff-rub">'+money(tier.rub)+'</div>'
        + '<div class="pp-tariff-note">Итоговая цена для оплаты через PlataPay</div>'
        + '<button class="pp-tariff-pick" data-tier="'+idx+'">Выбрать</button>'
        + '</div>';
    });
    html += '<div class="pp-tariff-card">'
      + '<div class="pp-tariff-name">Другой тариф / сумма</div>'
      + '<div class="pp-tariff-usd">Например, пополнение баланса или годовой тариф</div>'
      + '<div class="pp-tariff-rub">по запросу</div>'
      + '<div class="pp-tariff-note">Уточним сумму и пришлём ссылку на оплату</div>'
      + '<button class="pp-tariff-pick" data-tier="custom">Выбрать</button>'
      + '</div>';
    return html + '</div>';
  }
  function openTariffs(s){
    currentService=s;
    body.innerHTML = '<h3 class="pp-tariff-title">'+escapeHtml(s.name)+'</h3>'
      + '<p class="pp-tariff-sub">Выберите тариф. Цена считается автоматически: стоимость тарифа в долларах × '+RATE+' ₽ + '+FEE+' ₽.</p>'
      + tierCardsHtml(s);
  ...
  function checkoutHtml(s,tier){
    ...
    return '<h3 class="pp-tariff-title">Оформить оплату</h3>'
      + '<p class="pp-tariff-sub">Почта обязательна. Остальной способ связи можно выбрать дополнительно.</p>'
      + '<div class="pp-checkout-box">'
      + '<div class="pp-checkout-summary"><strong>'+escapeHtml(s.name)+' — '+escapeHtml(tierName)+'</strong><span>Цена: '+escapeHtml(priceText)+'</span></div>'
      + '<form id="ppCheckoutForm">'
      + '<div class="pp-checkout-field"><label>Email <b style="color:#ff9b9b">*</b></label><input id="ppCheckoutEmail" type="email" placeholder="name@example.com" required autocomplete="email"></div>'
      + '<div class="pp-checkout-row"><div class="pp-checkout-field"><label>Доп. способ связи</label><select id="ppCheckoutChannel"><option value="">Не нужен</option><option>Telegram</option><option>WhatsApp</option><option>Телефон</option></select></div>'
      + '<div class="pp-checkout-field"><label>Контакт</label><input id="ppCheckoutContact" type="text" placeholder="@username или номер телефона"></div></div>'
      + '<button class="pp-checkout-submit" type="submit">Продолжить к оплате</button>'
      + '<div class="pp-checkout-error" id="ppCheckoutError"></div>'
      + '</form><button class="pp-checkout-back" id="ppCheckoutBack">← Назад к тарифам</button></div>';
  }
  function openCheckout(s,tier){
    ...
    form.addEventListener('submit',function(e){
      ...
      if(!/^\S+@\S+\.\S+$/.test(email)){
        err.textContent='Введите корректную почту'; err.style.display='block'; return;
      }
      var priceText = tier && tier.rub ? money(tier.rub) : 'по запросу';
      var tierName = tier ? tier.label : 'Другой тариф / сумма';
      ...
      if(srv && ctc && go){
        srv.value=serviceLine;
        ctc.value=contactLine;
        go.click();
        ...
        err.textContent='Форма временно недоступна. Напишите в Telegram: @Kimzar_A'; err.style.display='block';
      }
    });
  }
  ...
  document.addEventListener('click',function(e){
    ...
    if(!s) return;
    var payLink=e.target.closest('a');
    if(payLink || e.target.closest('.pp-pay,.pp-pc-pay')) e.preventDefault();
    openTariffs(s);
  }, true);
  hydrateCards();
  setTimeout(hydrateCards,500);
  setTimeout(hydrateCards,1500);
})();
</script>
`;
}
