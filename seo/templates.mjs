  ...
  return {
    ...
    body: `
      ...
      <section class="block">
        ...
        <p>${s.price ? `Минимальный заказ оплаты ${s.name} — ${fmtRubBare(s.price)}.` : `Стоимость оплаты ${s.name} зависит от суммы брони или счёта — фиксированного тарифа тут нет.`} В сумму уже входит наша комиссия и конвертация по курсу банка-эмитента. Никаких скрытых платежей: цену называем до оплаты.</p>
  ...
  return {
    ...
    body: `
      ...
      <section class="block">
        <h2>Тарифы ${s.name}</h2>
        ${tiersList}
        <p>${s.price ? `Минимальный заказ оплаты ${s.name} через нас — ${fmtRubBare(s.price)} за месяц. Годовые тарифы обычно выходят дешевле на 15–20%, потому что сервис сам даёт скидку при оплате на год.` : `Фиксированного месячного тарифа у ${s.name} нет — стоимость зависит от суммы брони или счёта. Точную цифру в рублях называем до оплаты, без скрытых комиссий.`}</p>
      </section>
