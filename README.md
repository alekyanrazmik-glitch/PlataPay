...
- `seo/` — генератор SEO-лендингов:
  ...
  - `oferta.mjs` — единый текст публичной оферты: подставляется в попап
    `#popupoferta` на всех страницах Тильды, рендерится как страница
    `/oferta/` и линкуется в подвале всех SEO-лендингов
